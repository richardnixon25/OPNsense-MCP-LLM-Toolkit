  ...
  private setupHandlers() {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        ...
        {
          name: 'wireguard_get_server',
          description: 'Get detailed configuration for a specific WireGuard server',
          inputSchema: {
            type: 'object',
            properties: {
              uuid: { type: 'string', description: 'Server UUID (from wireguard_list_servers)' }
            },
            required: ['uuid']
          }
        },

        // PF States / Firewall Diagnostics Tools
        {
          name: 'pf_states',
        ...
        {
          name: 'pf_query_states',
          description: 'Search/filter PF states by criteria (source, destination, interface)',
          inputSchema: {
            type: 'object',
            properties: {
              filter: { type: 'string', description: 'Filter expression (e.g., IP address, interface name)' }
            }
          }
        },
        {
          ...
          inputSchema: {
            type: 'object',
            properties: {
              filter: { type: 'string', description: 'Filter for states to kill (e.g., source IP, interface)' }
            }
          }
        },
        {
          name: 'pf_flush_states',
        ...
        },
        {
          ...
          inputSchema: {
            type: 'object',
            properties: {
              section: { type: 'string', description: 'Statistics section (optional - all if omitted)' }
            }
          }
        },
        {
          name: 'pf_delete_state',
          description: 'Delete a specific PF state by ID',
          inputSchema: {
            type: 'object',
            properties: {
              stateId: { type: 'string', description: 'State ID to delete' },
              creatorId: { type: 'string', description: 'Creator ID of the state' }
            },
        ...
        {
          name: 'alias_get_by_name',
          description: 'Get alias UUID by name',
          inputSchema: {
            type: 'object',
            properties: {
              name: { type: 'string', description: 'Alias name (e.g., TAILSCALE_REDIRECTORS)' }
            },
            required: ['name']
          }
        },
        {
          name: 'alias_list_content',
        ...
        {
          name: 'alias_add_entry',
          description: 'Add an IP/host entry to an alias',
          inputSchema: {
            type: 'object',
            properties: {
              alias: { type: 'string', description: 'Alias name' },
              entry: { type: 'string', description: 'IP address or hostname to add' }
            },
            required: ['alias', 'entry']
          }
        },
        {
          name: 'alias_delete_entry',
          description: 'Remove an IP/host entry from an alias',
          inputSchema: {
            type: 'object',
            properties: {
              alias: { type: 'string', description: 'Alias name' },
              entry: { type: 'string', description: 'IP address or hostname to remove' }
            },
            required: ['alias', 'entry']
          }
        }
...
${analysis.recommendations.length > 0 ? analysis.recommendations.map(r => `- ${r}`).join('\n') : '- No recommendations'}`
        ...
        case 'wireguard_status': {
          await this.ensureInitialized();
          try {
            const result = await this.client!.get('/wireguard/service/status');
            return {
              content: [{
                type: 'text',
            ...
            throw new McpError(
              ErrorCode.InternalError,
              `Failed to get WireGuard status: ${error.message}`
            );
          }
        }
        ...
        case 'wireguard_show': {
          await this.ensureInitialized();
          try {
            const result = await this.client!.get('/wireguard/service/show');
            return {
              content: [{
                type: 'text',
                text: JSON.stringify(result, null, 2)
            ...
            throw new McpError(
              ErrorCode.InternalError,
              `Failed to get WireGuard tunnel details: ${error.message}`
            );
          }
        }

        case 'wireguard_list_servers': {
          await this.ensureInitialized();
          try {
            const result = await this.client!.get('/wireguard/server/searchServer');
            return {
              content: [{
                type: 'text',
                text: JSON.stringify(result, null, 2)
            ...
            throw new McpError(
              ErrorCode.InternalError,
              `Failed to list WireGuard servers: ${error.message}`
            );
          }
        }

        case 'wireguard_list_clients': {
          await this.ensureInitialized();
          try {
            const result = await this.client!.get('/wireguard/client/searchClient');
            return {
              content: [{
                type: 'text',
                text: JSON.stringify(result, null, 2)
            ...
            throw new McpError(
              ErrorCode.InternalError,
              `Failed to list WireGuard clients: ${error.message}`
            );
          }
        }

        case 'wireguard_restart': {
          await this.ensureInitialized();
          try {
            const result = await this.client!.post('/wireguard/service/restart');
            return {
              content: [{
                type: 'text',
                text: JSON.stringify(result, null, 2)
            ...
            throw new McpError(
              ErrorCode.InternalError,
              `Failed to restart WireGuard: ${error.message}`
            );
          }
        }

        case 'wireguard_get_server': {
          await this.ensureInitialized();
          if (!args || !args.uuid) {
            throw new McpError(
              ErrorCode.InvalidRequest,
          ...
          try {
            const result = await this.client!.get(`/wireguard/server/getServer/${args.uuid}`);
            return {
              content: [{
                type: 'text',
                text: JSON.stringify(result, null, 2)
            ...
            throw new McpError(
              ErrorCode.InternalError,
              `Failed to get WireGuard server details: ${error.message}`
            );
          }
        }

        // PF States / Firewall Diagnostics Handlers
        case 'pf_states': {
          await this.ensureInitialized();
          try {
            const result = await this.client!.get('/diagnostics/firewall/pf_states');
            return {
              content: [{
                type: 'text',
                text: JSON.stringify(result, null, 2)
            ...
            throw new McpError(
              ErrorCode.InternalError,
              `Failed to get PF states: ${error.message}`
            );
          }
        }

        case 'pf_query_states': {
          await this.ensureInitialized();
          try {
            const filter = args?.filter || '';
            const result = await this.client!.post('/diagnostics/firewall/query_states', {
              searchPhrase: filter
            });
            return {
              content: [{
            ...
            throw new McpError(
              ErrorCode.InternalError,
              `Failed to query PF states: ${error.message}`
            );
          }
        }
        ...
        case 'pf_kill_states': {
          ...
          try {
            const filter = args?.filter || '';
            const result = await this.client!.post('/diagnostics/firewall/kill_states', {
              filter: filter
            });
            return {
              content: [{
            ...
            throw new McpError(
              ErrorCode.InternalError,
              `Failed to kill PF states: ${error.message}`
            );
          }
        }

        case 'pf_flush_states': {
          await this.ensureInitialized();
          try {
            const result = await this.client!.post('/diagnostics/firewall/flush_states');
            return {
              content: [{
                type: 'text',
                text: JSON.stringify(result, null, 2)
            ...
            throw new McpError(
              ErrorCode.InternalError,
              `Failed to flush PF states: ${error.message}`
            );
          }
        }
        ...
        case 'pf_statistics': {
          ...
          try {
            ...
            const url = section 
              ? `/diagnostics/firewall/pf_statistics/${section}`
            ...
            throw new McpError(
              ErrorCode.InternalError,
              `Failed to get PF statistics: ${error.message}`
            );
          }
        }

        case 'pf_delete_state': {
          await this.ensureInitialized();
          if (!args || !args.stateId || !args.creatorId) {
            throw new McpError(
              ErrorCode.InvalidRequest,
              'stateId and creatorId parameters are required'
            );
          }
          try {
            const result = await this.client!.post(`/diagnostics/firewall/del_state/${args.stateId}/${args.creatorId}`);
            return {
              content: [{
                type: 'text',
                text: JSON.stringify(result, null, 2)
            ...
            throw new McpError(
              ErrorCode.InternalError,
              `Failed to delete PF state: ${error.message}`
            );
          }
        }

        // Gateway Monitoring Handler
        case 'gateway_status': {
          await this.ensureInitialized();
          try {
            const result = await this.client!.get('/routes/gateway/status');
            return {
              content: [{
                type: 'text',
                text: JSON.stringify(result, null, 2)
        ...
        // Firewall Alias Handlers
        case 'alias_list': {
          await this.ensureInitialized();
          try {
            const result = await this.client!.get('/firewall/alias/searchItem');
            return {
              content: [{
                type: 'text',
                text: JSON.stringify(result, null, 2)
        ...
        case 'alias_get': {
          ...
          try {
            const result = await this.client!.get(`/firewall/alias/getItem/${args.uuid}`);
            return {
              content: [{
                type: 'text',
                text: JSON.stringify(result, null, 2)
            ...
            throw new McpError(
              ErrorCode.InternalError,
              `Failed to get alias details: ${error.message}`
            );
          }
        }

        case 'alias_get_by_name': {
          await this.ensureInitialized();
          if (!args || !args.name) {
            throw new McpError(
              ErrorCode.InvalidRequest,
          ...
          try {
            const result = await this.client!.get(`/firewall/alias/getAliasUUID/${args.name}`);
            return {
              content: [{
                type: 'text',
                text: JSON.stringify(result, null, 2)
            ...
            throw new McpError(
              ErrorCode.InternalError,
              `Failed to get alias by name: ${error.message}`
            );
          }
        }

        case 'alias_list_content': {
          await this.ensureInitialized();
          if (!args || !args.alias) {
            throw new McpError(
              ErrorCode.InvalidRequest,
              'alias parameter is required'
            );
          }
          try {
            const result = await this.client!.get(`/firewall/alias_util/list/${args.alias}`);
            return {
              content: [{
                type: 'text',
                text: JSON.stringify(result, null, 2)
            ...
            throw new McpError(
              ErrorCode.InternalError,
              `Failed to list alias content: ${error.message}`
            );
          }
        }

        case 'alias_add_entry': {
          await this.ensureInitialized();
          if (!args || !args.alias || !args.entry) {
            throw new McpError(
              ErrorCode.InvalidRequest,
              'alias and entry parameters are required'
            );
          }
          try {
            const result = await this.client!.post(`/firewall/alias_util/add/${args.alias}`, {
              address: args.entry
            });
            return {
              content: [{
            ...
            throw new McpError(
              ErrorCode.InternalError,
              `Failed to add alias entry: ${error.message}`
            );
          }
        }

        case 'alias_delete_entry': {
          await this.ensureInitialized();
          if (!args || !args.alias || !args.entry) {
            throw new McpError(
              ErrorCode.InvalidRequest,
          ...
          try {
            const result = await this.client!.post(`/firewall/alias_util/delete/${args.alias}`, {
              address: args.entry
            });
            return {
              content: [{
            ...
            throw new McpError(
              ErrorCode.InternalError,
              `Failed to delete alias entry: ${error.message}`
            );
          }
        }
